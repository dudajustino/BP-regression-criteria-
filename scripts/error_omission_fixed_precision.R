...
crit <- array(0, dim=c(NREP,12,12))
select_correct <- matrix(0, nrow=12, ncol=3)

# True values of the parameters
beta <- c(1.1, 0.8, 1.3, 1.2, 1.3)
...
for(l in 1:3){
  ...
  while(i <= NREP)
  {
    ...
    tryCatch({
      # correct model
      fit4 <- gamlss(y~x1+x2+x3+x4,family = BP(mu.link = "log",sigma.link = "identity"), trace=FALSE)

      # covariate omission specificatio
      fit1 <- gamlss(y~x1,family = BP(mu.link = "log",sigma.link = "identity"), trace=FALSE)
      fit2 <- gamlss(y~x1+x2,family = BP(mu.link = "log",sigma.link = "identity"), trace=FALSE)
      fit3 <- gamlss(y~x1+x2+x3,family = BP(mu.link = "log",sigma.link = "identity"), trace=FALSE)
      ...
      if(fit1$converged == TRUE && fit2$converged == TRUE && fit3$converged == TRUE && fit4$converged == TRUE)
      {
        ...
        for(j in 1:12){
          ...
          if(escolhido == 4){
            select_correct[j, l] <- select_correct[j, l] + 1
          }
        }
...
mean_criterios <- colMeans(crit[,,1:12])
...
colnames(prop_correct) <- c("C1","C2", "C3")
